...
object Lab7
{
  ...
  def main(args: Array[String])
  {
    ...
    var par = Array(4.0, 3.0, 5.0, 3.0, 4.0, 4.0, 5.0, 3.0, 5.0, 4.0, 4.0, 5.0, 5.0, 3.0, 4.0, 4.0, 3.0, 4.0)
    ...
    val score = Container(vars:_*).reduce(_+_)
    // for a)
    def score_above_80(s: Int) = s >= 80
    // for b)
    def skill_above_03(s: Double) = s >= 0.3
    // constraint that skill must be above 0.3
    skill.addConstraint(s => if (s >= 0.3) 1 ;
    else 0 )
    // a)
    ...
    val p1_shots =  Array.tabulate(18)((hole: Int) => Chain(p1_skill, (s: Double) =>
      Select(s/8  -> (par(hole)-2),
        s/2  -> (par(hole)-1),
        s -> par(hole),
        (4 /5 ) * (1  - (s * 13)/8 ) -> (par(hole)+1),
        (1 /5 ) * (1  - (s * 13)/8 ) -> (par(hole)+2))))
    val p2_shots =  Array.tabulate(18)((hole: Int) => Chain(p2_skill, (s: Double) =>
      Select(s/8  -> (par(hole)-2),
        s/2  -> (par(hole)-1),
        s -> par(hole),
        (4/5) * (1  - (s * 13)/8) -> (par(hole)+1),
    ...
    def skill_above_0(s: Int) = s > 0
    p1_skill.observe(0.5)
    p2_skill.observe(0.5)
    println(Importance.probability(total, skill_above_0 _))
  }
}
