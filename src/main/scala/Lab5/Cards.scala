...
object Cards {
    def main(args: Array[String]) {
        ...
        // Three out of the 20 x 4 = 80 possible configurations (B1 =  bet placed by player 1, B2 = bet placed by player 2
        //
        //        B1 | B2
        //        ___|___
        //        0  | 0
        ...
        val config_1 =  // Only player1 bets
            Apply(player1Bet1,player2Bet,
                ((p1Bet: Boolean, p2Bet: Boolean) => p1Bet && !p2Bet))
        println(VariableElimination.probability(config_1, true))

        val config_2 =  // Only player2 bets
            Apply(player1Bet1,player2Bet,
                ((p1Bet: Boolean, p2Bet: Boolean) => !p1Bet && p2Bet))
        println(VariableElimination.probability(config_2, true))

        val config_3 = // Both players bet
            Apply(player1Bet1,player2Bet,
                ((p1Bet: Boolean, p2Bet: Boolean) => p1Bet && p2Bet))
        println(VariableElimination.probability(config_3, true))
    }
